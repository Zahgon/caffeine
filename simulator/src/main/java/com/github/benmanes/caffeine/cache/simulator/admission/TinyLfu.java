            ...
            case "count-min-4" ->
                {
                    ...
                    yield switch(reset.toLowerCase(US)) {
                        case "climber" ->
                            new ClimberResetCountMin4(settings.config());
                        case "periodic" ->
                            new PeriodicResetCountMin4(settings.config());
                        case "indicator" ->
                            new IndicatorResetCountMin4(settings.config());
                        case "incremental" ->
                            new IncrementalResetCountMin4(settings.config());
                        default ->
                            throw new IllegalStateException("Unknown reset type: " + reset);
                    };
                }
            case "tiny-table" ->
                new TinyCacheAdapter(settings.config());
            case "count-min-64" ->
                new CountMin64TinyLfu(settings.config());
            case "perfect-table" ->
                new PerfectFrequency(settings.config());
            case "random-table" ->
                new RandomRemovalFrequencyTable(settings.config());
            default ->
