...
public abstract class TextReporter implements Reporter {
    ...
    private ImmutableList<PolicyStats> getSortedStats(Collection<PolicyStats> results) {
        String sortBy = results.stream().flatMap(policyStats -> policyStats.metrics().keySet().stream()).filter(header -> header.toLowerCase(US).equals(settings.report().sortBy().toLowerCase(US))).findAny().orElseThrow(() -> new IllegalArgumentException("Unknown sort order: " + settings.report().sortBy()));
    ...
    private ImmutableSet<String> getHeaders(Collection<PolicyStats> results) {
        var columns = results.stream().flatMap(policyStats -> policyStats.metrics().values().stream()).filter(metric -> metric.characteristics().isEmpty() || metric.characteristics().stream().anyMatch(characteristics::contains)).filter(metric -> metric.required() || !metrics().format(metric).isEmpty()).map(Metric::name).collect(toImmutableSet());
        return results.stream().flatMap(policyStats -> policyStats.metrics().keySet().stream()).filter(columns::contains).collect(toImmutableSet());
    }
     ...
     */
    protected abstract void write(Writer writer, Set<String> headers, List<PolicyStats> results) throws IOException;
